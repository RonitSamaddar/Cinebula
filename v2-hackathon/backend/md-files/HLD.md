...
2. Top-genres — return a user's ranked genres with priority weights.
3. Data service — proxy `/api/movies` to TKACR and append a computed `priority` score to every movie.
4. Priority — IMDb Bayesian weighted rating, normalised 0–100, computed per response.
...
│   │
│   ├── user/
│   │   ├── handler.go           # GET /user/profile
│   │   ├── model.go             # User struct
│   │   └── store.go             # read databases/users.csv
│   │
│   ├── top-genres/
│   │   ├── handler.go           # GET /top-genres?userId=N
...
| GET    | /user/profile   | JWT  | Return user profile                             |
| GET    | /top-genres     | —    | User’s top-9 genres with priority weights       |
